...
post '/age' do
  ...
  age = "#{response.face_details[0].age_range.low} - #{response.face_details[0].age_range.high}"
  gender = response.face_details[0].gender.value
  emotions = ""
  for i in 0..(response.face_details[0].emotions.count - 1)
    type = response.face_details[0].emotions[i].type
    confidence = response.face_details[0].emotions[i].confidence
    emotions = "#{emotions}, #{type} - #{confidence}"
  end
  data = {
    age: age,
    gender: gender,
